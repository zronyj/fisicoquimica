...
# *****************************************
# Funcion para graficar los orbitales del
# atomo de hidrogeno en 3 dimensiones
# *****************************************
def orbital3D(n=1, l=0, m=0, Z=1, d=[-4,4,100], g="surf"):
    try:
        import ipyvolume as ipy
    except ImportError as e:
        print("No se encuentra la libreria: IPyVolume")
        return None
    x = np.linspace(d[0], d[1], d[2])
    y = np.linspace(d[0], d[1], d[2])
    z = np.linspace(d[0], d[1], d[2])
    Xi, Yi, Zi = np.meshgrid(x, y, z)
    O = Psi2(x=Xi, y=Yi, z=Zi, n=n, l=l, m=m, Z=Z)
    ipy.clear()
    ipy.Figure(controls=True)
    if g == "surf":
        ipy.pylab.plot_isosurface(O,level=O.mean()*1.5,color=(0,0.5,0.9,1.0))
    else:
        ipy.volshow(O, level=[0.7,0.5,0.3], opacity=[0,0.03,0.06],
